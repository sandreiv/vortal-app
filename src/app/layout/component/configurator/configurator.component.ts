...
export class ConfiguratorComponent implements OnInit {
  ...
  applyTheme(type: 'primary' | 'surface', color: ColorEvent) {
    if (type === 'primary') {
      document.documentElement.style.setProperty(
        '--primary-color',
        color.palette?.['500'] || '#000000'
      )
    } else if (type === 'surface') {
      Object.entries(color.palette || {}).forEach(([key, value]) => {
        document.documentElement.style.setProperty(`--surface-${key}`, value)
      })
    }
  }
  ...
  onPresetChange(event: string) {
    ...
    // Aplicar la paleta de superficie
    if (surfacePalette) {
      Object.entries(surfacePalette).forEach(([key, value]) => {
        document.documentElement.style.setProperty(`--surface-${key}`, value)
      })
    }
  }
